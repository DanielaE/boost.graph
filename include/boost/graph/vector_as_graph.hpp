...
namespace boost {
  ...
  vertices(const std::vector<EdgeList, Alloc>& v)
  {
    ...
    return std::make_pair(Iter(0), Iter(typename EdgeList::value_type(v.size())));
  }
  ...
  {
    g.resize(g.size()+1);
    return static_cast<typename EdgeList::value_type>(g.size()-1);
  }
               ...
               std::vector<EdgeList, Allocator>& g)
  {
    ...
    for (std::size_t i = 0; i < g.size(); ++i)
      remove_edge(typename EdgeList::value_type(i), u, g);
  }
