...
namespace boost {
               ...
               const BinaryFunction& combine, const BinaryPredicate& compare)
    {
      ...
      // change.
      if ( compare(combine(d_u, static_cast<const D>(w_e)), d_v) ) {
        put(d, v, combine(d_u, static_cast<const D>(w_e)));
      ...
      } else if (is_undirected && compare(combine(d_v, static_cast<const D>(w_e)), d_u)) {
        put(d, u, combine(d_v, static_cast<const D>(w_e)));
